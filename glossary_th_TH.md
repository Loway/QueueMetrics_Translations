...
| Transcript | The resulting text of a call, as sent to the LLM (distinct from Transcription, the process) | บทถอดเสียง | `qa_ai_testdialog_transcript_hint`, `qa_ai_testdialog_hint` | Seeded from `qa_ai_testdialog_hint` ("บทถอดเสียงการโทร" = call transcript). Thai does not distinguish Transcript from Transcription; same noun reused for both. |
