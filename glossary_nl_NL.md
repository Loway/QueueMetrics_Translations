...
| Transcript | The resulting text of a call, as sent to the LLM (distinct from Transcription, the process) | Transcriptie | `qa_ai_testdialog_transcript_hint`, `qa_ai_testdialog_hint` | Pack doesn't distinguish Transcript from Transcription — both rendered "transcriptie" (seeded from `qa_ai_testdialog_hint`'s "voorbeeldtranscriptie"); no separate Transcription row exists yet in this glossary |
