...
| Transcript | The resulting text of a call, as sent to the LLM (distinct from Transcription, the process) | Transcription | `qa_ai_testdialog_transcript_hint`, `qa_ai_testdialog_hint` | French pack does not distinguish Transcript from Transcription; reuses the same word (seeded from `qa_ai_testdialog_hint`: *transcription d'appel*) |
