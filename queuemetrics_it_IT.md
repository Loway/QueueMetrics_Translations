...
## ART?
    ...
    art_deprecated_page=?Avviso: È ora disponibile una nuova pagina agente. Quella che stai usando sarà rimossa nella prossima versione. Chiedi al tuo amministratore di migrarti alla pagina agente aggiornata.
...
## Headers
    ...
    hdr_dbtest_ephemeral_attribute_inspector=?Vedi Attr. effimeri
    ...
    hdr_transcription_servers=?Server di trascrizione
...
### Cases
    ...
    edit_record_qa_testaidialog=Tester dialogo AI
    ...
    edit_record_qaitems_explanation=Spiegazione:
    ...
    edit_transcription_servers_title=?Modifica server di trascrizione
    # 👽 edit_transcription_go_live_status -> Go to live view
    edit_transcription_go_live_status=?Vai alla vista live
...
## Quality Assessments
    ...
    qa_grading_no_valid_targets=Inserire almeno un target di valutazione
...
### Automated AI grading
    ...
    qa_ai_grading=Valutazione AI
    qa_ai_testdialog_col_comment=Commento
    qa_ai_testdialog_col_item=Oggetto
    qa_ai_testdialog_col_value=Valore
    qa_ai_testdialog_hint=Questa è la richiesta inviata al motore LLM, su una trascrizione di chiamata di esempio. Modificala e riprova per vedere come reagisce il modello.
    ...
    qa_ai_testdialog_prompt_hint=?Prompt: questa parte dipende solo dal modulo QA, quindi è la stessa per ogni chiamata e il motore LLM potrebbe memorizzarla nella cache.
    qa_ai_testdialog_request=Richiesta
    qa_ai_testdialog_response=Risposta
    qa_ai_testdialog_title=Tester dialogo AI
    # 👽 qa_ai_testdialog_transcript_hint -> Transcript: this part changes with every call, so it is sent separately and never cached.
    qa_ai_testdialog_transcript_hint=?Trascrizione: questa parte cambia per ogni chiamata, quindi viene inviata separatamente e non è mai memorizzata nella cache.
    qa_ai_testdialog_try=Prova
    qa_grader_col_agent=Agente
    qa_grader_col_call_duration=Durata chiamata
    qa_grader_col_caller=Chiamante
    qa_grader_col_datetime=Data/ora
    qa_grader_col_queue=Coda
    qa_grader_col_score=Punteggio
    ...
    qa_grader_stat_evaluated=Valutate
    qa_grader_stat_evaluation_errors=Errori di valutazione
    qa_grader_stat_maximum_score=Punteggio max.
    qa_grader_stat_minimum_score=Punteggio min.
    qa_grader_stat_tokens_used=Token usati
...
Description of configuration properties
    ...
    propedit_key_maxGatheringTime=?Imposta il tempo totale massimo consentito per il processo di raccolta ICE. Se impostato a 0 l'ottimizzazione è disabilitata.
    ...
    propedit_key_maxPostGatheringTime=?La durata del timeout soft attivato dopo il rilevamento di un candidato STUN/TURN instradabile.
    ...
    propedit_category_ai_integration=Integrazione AI
    propedit_feature_ai_integration_settings=Impostazioni generali
    propedit_key_llm_captureUniqueId_desc=Questa espressione regolare viene usata per estrarre l'UniqueId della chiamata dal nome del file. Es. audio-(.+).wav cattura l'uniqueid 1234.1235 dal nome file audio-1234.1235.wav. Se lasciato vuoto, non viene catturato alcun UniqueId
    propedit_key_llm_verbose_desc=Stampa l'input e l'output da/verso il modello LLM nel log di sistema
    propedit_key_qa_minPrcAllCalls_desc=Valore predefinito della percentuale minima di chiamate valutate per tutte le chiamate nella pagina del Grader
...
Description of security keys:
    ...
    keydesc_nodialout=?L'utente non è autorizzato a effettuare chiamate in uscita
    ...
    keydesc_queue_login_assigned_only=?Gli utenti possono accedere solo alle code assegnate
    ...
    keydesc_transcription_server=L'utente può visualizzare e aggiornare i server di trascrizione
    ...
    keydesc_ephemeral_read=?Gli utenti possono leggere dalla tabella dei dati effimeri
    # 👽 keydesc_ephemeral_write -> Users can write to the ephemeral data table
    keydesc_ephemeral_write=?Gli utenti possono scrivere nella tabella dei dati effimeri
...
### Softphone
    ...
    phone_audio_mic_not_found=?Nessun microfono rilevato
    # 👽 phone_audio_settings -> Audio Settings
    phone_audio_settings=?Impostazioni Audio
    # 👽 phone_audio_device_not_found -> Audio device not found
    phone_audio_device_not_found=?Dispositivo audio non trovato
    ...
    phone_hid_close_device_err=?Errore durante la chiusura del dispositivo HID
    # 👽 phone_hid_device_already_open -> HID device already open
    phone_hid_device_already_open=?Dispositivo HID già aperto
    # 👽 phone_hid_device_connected -> HID device connected
    phone_hid_device_connected=?Dispositivo HID connesso
    # 👽 phone_hid_disconnected_device -> Disconnected HID device
    phone_hid_disconnected_device=?Dispositivo HID disconnesso
    # 👽 phone_hid_open_device -> Select a HID device
    phone_hid_open_device=?Seleziona un dispositivo HID
    # 👽 phone_hid_open_device_err -> Error occurred while opening HID device
    phone_hid_open_device_err=?Errore durante l'apertura del dispositivo HID
    # 👽 phone_selected_audio_device -> Selected audio device
    phone_selected_audio_device=?Dispositivo audio selezionato
    # 👽 phone_selected_hid_device -> Selected HID device
    phone_selected_hid_device=?Dispositivo HID selezionato
    # 👽 phone_selected_mic_device -> Selected microphone device
    phone_selected_mic_device=?Microfono selezionato
    # 👽 phone_selected_ringtone_device -> Selected ringtone device
    phone_selected_ringtone_device=?Suoneria selezionata
    # 👽 phone_set_audio_info -> Changed audio device
    phone_set_audio_info=?Dispositivo audio modificato
    # 👽 phone_set_mic_info -> Changed mic device
    phone_set_mic_info=?Microfono modificato
    # 👽 phone_set_audio_err -> Error occurred while setting audio device
    phone_set_audio_err=?Errore durante la modifica del dispositivo audio
    # 👽 phone_ringtone -> Ringtone    
    phone_ringtone=?Suoneria
...
## Homepage
    ...
    home_configuration_transcription_servers=?Server di trascrizione
    ...
    home_dbtest_ephemeral_attribute_inspector=?Ispettore Attr. effimeri
    ...
    home_transcription_title=?Trascrizioni live
    # 👽 home_transcription_subtitle -> View of Live Transcriptions
    home_transcription_subtitle=?Vista delle trascrizioni live
...
Other items
    ...
    err_not_blank=?Il campo non può essere lasciato vuoto.
    ...
    graph_position=?Posizione Grafico
    # 👽 graph_position_top -> Top
    graph_position_top=?Alto
    # 👽 graph_position_bottom -> Bottom
    graph_position_bottom=?Basso
    # 👽 graph_position_left -> Left
    graph_position_left=?Sinistra
    # 👽 graph_position_right -> Right
    graph_position_right=?Destra
    # 👽 graph_size -> Graph Size
    graph_size=?Dimensione Grafico
    # 👽 graph_size_big -> Big
    graph_size_big=?Grande
    # 👽 graph_size_normal -> Normal
    graph_size_normal=?Normale
    # 👽 graph_size_small -> Small
    graph_size_small=?Piccolo
    # 👽 graph_size_extra_small -> Extra Small
    graph_size_extra_small=?Extra Piccolo
    grp_ivr=Eventi Ivr
    ...
    licence_licence_expiring=La licenza sta per scadere
    ...
    licence_software_new_release_available=Nuova versione disponibile:
    ...
    qap_agent_license_error=Limite di agenti su licenza superato. Contattare il supervisore.
    qap_amo=AMO
    ...
    qap_agent_not_present_on_queue=?L'agente non è presente sulla coda outbound selezionata.
    qap_none=Nessuno
    ...
    qap_extended_status=?Stato esteso
    # 👽 qap_number_dialed -> Number dialed
    qap_number_dialed=?Numero composto
    # 👽 qap_wbt_attempt -> Attempt
    qap_wbt_attempt=?Tentativo
    # 👽 qap_wbt_recall_attempt -> Recall attempts
    qap_wbt_recall_attempt=?Tentativi di richiamo
    # 👽 qap_wombat_call_history -> Call history
    qap_wombat_call_history=?Cronologia chiamate
    # 👽 qap_wombat_dial -> Dial
    qap_wombat_dial=?Componi
    # 👽 qap_wombat_ext_status_success -> Extended status added successful
    qap_wombat_ext_status_success=?Stato esteso aggiunto con successo
    # 👽 qap_wombat_number_dial -> Number to dial
    qap_wombat_number_dial=?Numero da comporre
    # 👽 qap_wombat_out_attribute_success -> Attribute added successful
    qap_wombat_out_attribute_success=?Attributo aggiunto con successo
    # 👽 qap_wombat_reserve -> Reserve
    qap_wombat_reserve=?Riserva
    # 👽 qap_wombat_rs_agent -> Retry Requested
    qap_wombat_rs_agent=?Nuovo tentativo richiesto
    # 👽 qap_wombat_rs_agentskip -> Agent Skip
    qap_wombat_rs_agentskip=?Salto agente
    # 👽 qap_wombat_rs_blacklist -> Number Blacklisted
    qap_wombat_rs_blacklist=?Numero in blacklist
    # 👽 qap_wombat_rs_busy -> Line Busy
    qap_wombat_rs_busy=?Linea occupata
    # 👽 qap_wombat_rs_err -> Technical Error
    qap_wombat_rs_err=?Errore tecnico
    # 👽 qap_wombat_rs_lost -> Call Lost
    qap_wombat_rs_lost=?Chiamata persa
    # 👽 qap_wombat_rs_noagent -> Agent Timeout
    qap_wombat_rs_noagent=?Timeout agente
    # 👽 qap_wombat_rs_noanswer -> No Answer
    qap_wombat_rs_noanswer=?Nessuna risposta
    # 👽 qap_wombat_rs_number -> Invalid Number
    qap_wombat_rs_number=?Numero non valido
    # 👽 qap_wombat_rs_rejected -> Network Reject
    qap_wombat_rs_rejected=?Rifiuto di rete
    # 👽 qap_wombat_rs_terminated -> Call Completed
    qap_wombat_rs_terminated=?Chiamata completata
    # 👽 qap_wombat_rs_timeout -> Max Duration
    qap_wombat_rs_timeout=?Durata massima
    # 👽 qap_wombat_skip -> Skip
    qap_wombat_skip=?Salta
    # 👽 qap_wombat_update -> Update
    qap_wombat_update=?Aggiorna
    # 👽 qap_wombat_update_disable_msg -> An extended status must be selected
    qap_wombat_update_disable_msg=?È necessario selezionare uno stato esteso
    # 👽 qap_generic_error -> Error occured in WombatDialer
    qap_generic_error=?Si è verificato un errore in WombatDialer
    # 👽 qap_agent_not_available -> Agent not available
    qap_agent_not_available=?Agente non disponibile
    # 👽 qap_dial_success -> Dialed
    qap_dial_success=?Composto
    # 👽 qap_skip_success -> Skipped
    qap_skip_success=?Saltato
    # 👽 qap_no_call_to_reserve -> No call available for reservation
    qap_no_call_to_reserve=?Nessuna chiamata disponibile per la prenotazione
    # 👽 qap_agent -> Agent
    qap_agent=?Agente
    # 👽 qap_id_not_found -> No such ID
    qap_id_not_found=?ID inesistente
    # 👽 qap_id_found_wrong_state -> The ID exists but the call is in a wrong state
    qap_id_found_wrong_state=?L'ID esiste ma la chiamata è in uno stato errato
    # 👽 ephatt_attribute_character_error -> Attribute name cannot contain the '.' character
    ephatt_attribute_character_error=?Il nome dell'attributo non può contenere il carattere '.'
    # 👽 rt3_att_text -> Ephemeral - Text
    rt3_att_text=?Effimero - Testo
    # 👽 rt3_nofstate_unix_wbt -> Ephemeral - Time
    rt3_nofstate_unix_wbt=?Effimero - Tempo
    # 👽 rt3_circle_wbt_reservable -> Reservable calls
    rt3_circle_wbt_reservable=?Chiamate prenotabili
    # 👽 rt3_circle_wbt_incomplete -> Incomplete calls
    rt3_circle_wbt_incomplete=?Chiamate incomplete
    # 👽 rt3_nofstate_wbt -> Ephemeral - Value
    rt3_nofstate_wbt=?Effimero - Valore
    # 👽 rt3_wbt_campaign_stats -> Campaign stats
    rt3_wbt_campaign_stats=?Statistiche campagna
    # 👽 rt3_wbt_camp_est -> Attempts remaining
    rt3_wbt_camp_est=?Tentativi rimanenti
    # 👽 rt3_attributes -> Ephemeral Attribute
    rt3_attributes=?Attributo effimero
    # 👽 rt3_display_value -> Visualization mode
    rt3_display_value=?Modalità di visualizzazione
    # 👽 rt3_maxvalue -> Max value
    rt3_maxvalue=?Valore massimo
    # 👽 rt3_md_checkbox -> Is markdown
    rt3_md_checkbox=?È markdown
    # 👽 rt3_normal_number -> Normal
    rt3_normal_number=?Normale
    # 👽 rt3_normal_perc -> Percentage
    rt3_normal_perc=?Percentuale
    ...
    reports_email_not_valid=?Inserire indirizzi email validi
    ...
    reports_exportjob_create=?Crea Nuovo Job di Esportazione
    # 👽 reports_exportjob_create_successful -> Export job created successfully
    reports_exportjob_create_successful=?Job di esportazione creato con successo
    # 👽 reports_exportjob_error_create -> Error Creating Export Job:
    reports_exportjob_error_create=?Errore nella Creazione del Job di Esportazione:
    # 👽 reports_exportjob_title -> Existing jobs for this report
    reports_exportjob_title=?Job esistenti per questo report
    # 👽 reports_export_job -> Export
    reports_export_job=?Esporta
    ...
    reports_filter_pausedurmax=?Durata Pausa Max.
    # 👽 reports_filter_pausedurmin -> Min. Pause Duration
    reports_filter_pausedurmin=?Durata Pausa Min.
    # 👽 reports_filter_pausenmax -> Max. N Pause
    reports_filter_pausenmax=?N Pause Max.
    # 👽 reports_filter_pausenmin -> Min. N Pause
    reports_filter_pausenmin=?N Pause Min.
    # 👽 reports_filter_sessiondurmax -> Max. Session Duration
    reports_filter_sessiondurmax=?Durata Sessione Max.
    # 👽 reports_filter_sessiondurmin -> Min. Session Duration
    reports_filter_sessiondurmin=?Durata Sessione Min.
    # 👽 reports_filter_sessionext -> Session Ext.
    reports_filter_sessionext=?Sessione Ext.
    ...
    reports_save_report_title_exception=?Esiste già un report con lo stesso nome. Seleziona un nome diverso.
    reports_save_successful=Report salvato con successo
    # 👽 reports_scheduled_job -> Scheduled
    reports_scheduled_job=?Programmato
    ...
    reports_qa_noteform_save=?Salva nota
    # 👽 reports_qa_nonscoring -> Non-Scoring Evaluation
    reports_qa_nonscoring=?Valutazione senza punteggio
    # 👽 report_tab_page_list -> Page List
    report_tab_page_list=?Elenco pagine
    # 👽 reports_delete_filter_text -> Delete Filter
    reports_delete_filter_text=?Elimina filtro
    # 👽 reports_edit_filter_text -> Edit Filter
    reports_edit_filter_text=?Modifica filtro
    ...
    sync2_delete_cluster=?Elimina cluster
    ...
    timeline_download_chat_file=?Scarica file
    ...
    timeline_no_chats=?Nessuna chat disponibile
    ...
    title_do_fcrdo=Risoluzione alla Prima Chiamata
    title_do_groupingsdo=Raggruppamento Chiamate
    title_do_ivrdo=Chiamate IVR
    title_do_kodo=Chiamate Senza Risposta
    title_do_okdo=Chiamate Risposte
    title_do_outcomesdo=Chiamate con Esito
