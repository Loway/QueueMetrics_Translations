...
## ART?
    ...
    art_deprecated_page=?Avis : Une nouvelle page agent est maintenant disponible. Celle que vous utilisez actuellement sera supprimée dans la prochaine version. Veuillez demander à votre administrateur de vous migrer vers la page agent mise à jour.
...
## Headers
    ...
    hdr_transcription_servers=?Serveurs de transcription
...
### Cases
    ...
    edit_record_qa_testaidialog=?Tester le dialogue IA
    ...
    edit_record_qaitems_explanation=?Explication:
    ...
    edit_record_transcription_channel_remote_media_file=?Fichier média distant
    # 👽 edit_record_transcription_channel_state -> State
    edit_record_transcription_channel_state=?État
    # 👽 edit_record_transcription_channel_reserverd_at -> Reserved At
    edit_record_transcription_channel_reserverd_at=?Réservé le
    # 👽 edit_record_transcription_channel_last_seen -> Last Seen
    edit_record_transcription_channel_last_seen=?Dernière activité
    # 👽 edit_record_transcription_channel_tokens_in -> Tokens Input
    edit_record_transcription_channel_tokens_in=?Tokens en entrée
    # 👽 edit_record_transcription_channel_tokens_out -> Tokens Output
    edit_record_transcription_channel_tokens_out=?Tokens en sortie
    ...
    edit_record_transcription_server_channels=?Canaux
    # 👽 edit_record_transcription_server_priority -> Priority
    edit_record_transcription_server_priority=?Priorité
    ...
    edit_record_transcription_server_auth_token=?Jeton
    # 👽 edit_record_transcription_server_description -> Description
    edit_record_transcription_server_description=?Description
    # 👽 edit_record_transcription_server_driver_parms -> Parameters
    edit_record_transcription_server_driver_parms=?Paramètres
    # 👽 edit_record_transcription_server_status -> Status
    edit_record_transcription_server_status=?Statut
    # 👽 edit_record_transcription_server_healthy -> Healthy
    edit_record_transcription_server_healthy=?Sain
    # 👽 edit_record_transcription_server_last_health -> Last Health Check
    edit_record_transcription_server_last_health=?Dernier contrôle d'intégrité
    ...
    edit_transcription_servers_title=?Modifier les serveurs de transcription
    # 👽 edit_transcription_go_live_status -> Go to live view
    edit_transcription_go_live_status=?Aller à la vue en direct
...
## Quality Assessments
    ...
    qa_grading_no_valid_targets=?Veuillez saisir au moins une cible d'évaluation
...
### Automated AI grading
    ...
    qa_ai_grading=?Évaluation IA
    # 👽 qa_ai_testdialog_col_comment -> Comment
    qa_ai_testdialog_col_comment=?Commentaire
    ...
    qa_grader_col_call_duration=?Durée de l'appel
    # 👽 qa_grader_col_caller -> Caller
    qa_grader_col_caller=?Appelant
    # 👽 qa_grader_col_datetime -> Date/time
    qa_grader_col_datetime=?Date/heure
    # 👽 qa_grader_col_queue -> Queue
    qa_grader_col_queue=?File
    # 👽 qa_grader_col_score -> Score
    qa_grader_col_score=?Score
    # 👽 qa_grader_col_tokens -> Tokens
    qa_grader_col_tokens=?Tokens
    # 👽 qa_grader_col_transcription_duration -> Scoring time
    qa_grader_col_transcription_duration=?Temps d'évaluation
    # 👽 qa_grader_no_more_calls -> No other calls found
    qa_grader_no_more_calls=?Aucun autre appel trouvé
    # 👽 qa_grader_notrack_err -> You need key QA_TRACK to access this page
    qa_grader_notrack_err=?Vous devez posséder la clef QA_TRACK pour accéder à cette page
    # 👽 qa_grader_stat_attempted -> Attempted
    qa_grader_stat_attempted=?Tentés
    # 👽 qa_grader_stat_average_score -> Average score
    qa_grader_stat_average_score=?Score moyen
    # 👽 qa_grader_stat_calls_loaded -> Calls loaded
    qa_grader_stat_calls_loaded=?Appels chargés
    # 👽 qa_grader_stat_evaluated -> Evaluated
    qa_grader_stat_evaluated=?Évalués
    # 👽 qa_grader_stat_evaluation_errors -> Evaluation errors
    qa_grader_stat_evaluation_errors=?Erreurs d'évaluation
    # 👽 qa_grader_stat_maximum_score -> Max. score
    qa_grader_stat_maximum_score=?Score max.
    # 👽 qa_grader_stat_minimum_score -> Min. score
    qa_grader_stat_minimum_score=?Score min.
    # 👽 qa_grader_stat_tokens_used -> Tokens used
    qa_grader_stat_tokens_used=?Tokens utilisés
...
## Wallboard
    ...
    rt3_filters=?Afficher/Masquer Filtres
    rt3_go=Entrer
    rt3_gohome=Accueil
    rt3_goto_login=S'authentifier
    # 👽 rt3_goto_view -> Go to View
    rt3_goto_view=?Aller à la vue
    rt3_green=Vert
    ...
    rt3_manual_insert=?Insertion manuelle
...
Description of configuration properties
    ...
    propedit_key_agent_leaveAllQueuesOnLogoff_desc=?Les agents seront automatiquement déconnectés de toutes les files d'attente à la déconnexion
    ...
    propedit_key_maxGatheringTime=?Définit le temps total maximal autorisé pour le processus de collecte ICE. Si la valeur est 0, l'optimisation est désactivée.
    ...
    propedit_key_maxPostGatheringTime=?La durée du délai d'attente souple déclenché après la détection d'un candidat STUN/TURN routable.
    ...
    propedit_category_ai_integration=?Intégration IA
    # 👽 propedit_feature_ai_integration_settings -> General Settings
    propedit_feature_ai_integration_settings=?Paramètres généraux
    ...
    propedit_key_llm_captureUniqueId_desc=?Cette expression régulière est utilisée pour extraire l'UniqueId de l'appel à partir du nom de fichier. Par exemple, audio-(.+).wav capturera l'uniqueid 1234.1235 à partir du nom de fichier audio-1234.1235.wav. Si laissé vide, aucun UniqueId n'est capturé
    # 👽 propedit_key_llm_verbose_desc -> Print input and output from/to the LLM model to the system log
    propedit_key_llm_verbose_desc=?Écrit les entrées et sorties du modèle LLM dans le journal système
    # 👽 propedit_key_qa_minPrcAllCalls_desc -> Default value of mininum percentage of calls scored for All calls on Grader's page
    propedit_key_qa_minPrcAllCalls_desc=?Valeur par défaut du pourcentage minimum d'appels notés pour « Tous les appels » sur la page du Classeur
...
Description of security keys:
    ...
    keydesc_qa_chat_read=?Permet aux utilisateurs de lire la discussion et la transcription de l'interaction
    # 👽 keydesc_qa_chat_edit -> Allows users to edit the interaction's chat and transcription
    keydesc_qa_chat_edit=?Permet aux utilisateurs de modifier la discussion et la transcription de l'interaction
    # 👽 keydesc_qa_chat_delete -> Allows users to delete the interaction's chat and transcription
    keydesc_qa_chat_delete=?Permet aux utilisateurs de détruire la discussion et la transcription de l'interaction
    # 👽 keydesc_qa_chat_write -> Allows users to load the interaction's chat or transcription
    keydesc_qa_chat_write=?Permet aux utilisateurs de charger la discussion ou la transcription de l'interaction
    ...
    keydesc_queue_login_assigned_only=?Les utilisateurs ne peuvent se connecter qu'aux files qui leur sont assignées
    ...
    keydesc_transcription_server=?L'utilisateur peut visualiser et mettre à jour les serveurs de transcription
...
### Softphone
    ...
    phone_mute_toggle=?Couper/Réactiver le micro
    ...
    phone_hid_disconnect_device=?Déconnecter le périphérique HID
...
## Homepage
    ...
    home_configuration_transcription_servers=?Serveurs de transcription
    ...
    home_master_key_error=?Masterkey activée — utilisation recommandée pour les tests uniquement
    ...
    home_transcription_title=?Transcriptions en direct
    # 👽 home_transcription_subtitle -> View of Live Transcriptions
    home_transcription_subtitle=?Vue des transcriptions en direct
...
These are the titles of data blocks:
    ...
    teams_blk_external_callers=?Appelants externes
    # 👽 teams_blk_external_callees -> External Callees
    teams_blk_external_callees=?Appelés externes
    # 👽 teams_blk_external_callers_on_all -> External Callers (on all Calls)
    teams_blk_external_callers_on_all=?Appelants externes (sur tous les appels)
    # 👽 teams_blk_external_callees_on_all -> External Callees (on all Calls)
    teams_blk_external_callees_on_all=?Appelés externes (sur tous les appels)
    # 👽 teams_blk_caller_classification -> Teams Caller Classification
    teams_blk_caller_classification=?Classification des appelants Teams
    # 👽 teams_blk_callee_classification -> Teams Callee Classification
    teams_blk_callee_classification=?Classification des appelés Teams
...
These are kinds of calls:
    ...
    teams_kind_guest_user=?Utilisateur invité
    # 👽 teams_kind_ext_user -> External User
    teams_kind_ext_user=?Utilisateur externe
    # 👽 teams_kind_phone -> Phone
    teams_kind_phone=?Téléphone
    # 👽 teams_kind_other -> Other
    teams_kind_other=?Autre

These are localization labels for data blocks:

    # 👽 teams_caller_domain -> Caller domain
    teams_caller_domain=?Domaine de l'appelant
    # 👽 teams_n_calls -> N. Calls
    teams_n_calls=?Nb. Appels
    # 👽 teams_avg_wait -> Avg Wait
    teams_avg_wait=?Attente Moy.
    # 👽 teams_avg_talk -> Avg Talk
    teams_avg_talk=?Parole Moy.
    # 👽 teams_tot_talk -> Tot Talk
    teams_tot_talk=?Parole Tot.
    # 👽 teams_all_calls -> All Calls
    teams_all_calls=?Tous les appels
    # 👽 teams_guest_n_calls -> Guest N Calls
    teams_guest_n_calls=?Invité Nb Appels
    # 👽 teams_guest_avg_wait -> Guest Avg Wait
    teams_guest_avg_wait=?Invité Attente Moy.
    # 👽 teams_guest_avg_talk -> Guest Avg Talk
    teams_guest_avg_talk=?Invité Parole Moy.
    # 👽 teams_guest_tot_talk -> Guest Tot Talk
    teams_guest_tot_talk=?Invité Parole Tot.
    # 👽 teams_ext_n_calls -> Ext N Calls
    teams_ext_n_calls=?Ext Nb Appels
    # 👽 teams_ext_avg_wait -> Ext Avg Wait
    teams_ext_avg_wait=?Ext Attente Moy.
    # 👽 teams_ext_avg_talk -> Ext Avg Talk
    teams_ext_avg_talk=?Ext Parole Moy.
    # 👽 teams_ext_tot_talk -> Ext Tot Talk
    teams_ext_tot_talk=?Ext Parole Tot.
...
Other items
    ...
    err_not_blank=?La saisie ne peut pas être laissée vide.
    ...
    licence_licence_expiring=?Votre licence est sur le point d'expirer
    ...
    licence_software_new_release_available=?Nouvelle version disponible:
    ...
    qap_agent_license_error=?Dépasse la limite d'agents autorisée par la licence. Veuillez contacter votre superviseur.
    qap_amo=AMO
    ...
    qap_apply_feature_error=?Erreur lors de l'application de la fonctionnalité à l'interaction
    # 👽 qap_apply_outcome_error -> Error while applying outcome to interaction
    qap_apply_outcome_error=?Erreur lors de l'application du résultat à l'interaction
    ...
    qap_logout_message=?Déconnexion de toutes les files d'attente
    qap_memos=Mémos
    ...
    qap_agent_not_present_on_queue=?L'agent n'est pas présent sur la file d'attente sortante sélectionnée.
    qap_none=Aucun
    ...
    qap_wombat_rs_rejected=Appel rejeté (par le réseau)
    qap_wombat_rs_terminated=Appel terminé
    qap_wombat_rs_timeout=Durée maximale atteinte
    ...
    reports_filter_empty_hour_toggle=?Masquer les créneaux horaires vides
    ...
    reports_datablock_info_error=?Impossible de charger les informations du Bloc de Données
    ...
    reports_email_not_valid=?Veuillez saisir des adresses e-mail valides
    ...
    reports_exportjob_create=?Créer Nouveau Travail Export
    # 👽 reports_exportjob_create_successful -> Export job created successfully
    reports_exportjob_create_successful=?Travail d'export créé avec succès
    # 👽 reports_exportjob_error_create -> Error Creating Export Job:
    reports_exportjob_error_create=?Erreur Création Travail Export :
    # 👽 reports_exportjob_title -> Existing jobs for this report
    reports_exportjob_title=?Travaux existants pour ce rapport
    ...
    reports_filter_pausedurmax=?Durée Pause Max.
    # 👽 reports_filter_pausedurmin -> Min. Pause Duration
    reports_filter_pausedurmin=?Durée Pause Min.
    ...
    reports_filter_sessiondurmax=?Durée Session Max.
    # 👽 reports_filter_sessiondurmin -> Min. Session Duration
    reports_filter_sessiondurmin=?Durée Session Min.
    # 👽 reports_filter_sessionext -> Session Ext.
    reports_filter_sessionext=?Ext. Session
    ...
    reports_qa_grader_type=?Type de classeur
    ...
    reports_qa_tracked_by=?Évalué par
    ...
    reports_save_report_title_exception=?Un rapport avec le même nom existe déjà. Veuillez sélectionner un nom différent.
    reports_save_successful=Rapport sauvegardé avec Succès
    # 👽 reports_scheduled_job -> Scheduled
    reports_scheduled_job=?Planifié
    ...
    reports_qa_noteform_save=?Enregistrer la note
    # 👽 reports_qa_nonscoring -> Non-Scoring Evaluation
    reports_qa_nonscoring=?Évaluation sans notation
    # 👽 report_tab_page_list -> Page List
    report_tab_page_list=?Liste des pages
    # 👽 reports_delete_filter_text -> Delete Filter
    reports_delete_filter_text=?Supprimer le filtre
    # 👽 reports_edit_filter_text -> Edit Filter
    reports_edit_filter_text=?Modifier le filtre
    ...
    sync2_delete_cluster=?Supprimer le cluster
    ...
    timeline_chat_delete_confirm=?La Discussion sélectionnée sera détruite. Continuer?
    # 👽 timeline_chat_edit_speakers -> Edit Speakers
    timeline_chat_edit_speakers=?Modifier les intervenants
    # 👽 timeline_chat_segments_history -> View Edit History
    timeline_chat_segments_history=?Voir l'historique des modifications
    # 👽 timeline_chat_segments_delete -> Delete Chat
    timeline_chat_segments_delete=?Détruire la Discussion
    # 👽 timeline_chat_speaker -> Speaker
    timeline_chat_speaker=?Intervenant
    # 👽 timeline_chat_edited -> Edited
    timeline_chat_edited=?Modifié
    ...
    timeline_download_chat_file=?Télécharger le fichier
    ...
    timeline_no_chats=?Aucune discussion disponible
    ...
    timeline_tab_chats_speaker_error=?Les noms des intervenants doivent être uniques
    # 👽 timeline_tab_chats_no -> No
    timeline_tab_chats_no=?Non
    # 👽 timeline_tab_chats_yes -> Yes
    timeline_tab_chats_yes=?Oui
    timeline_totdur=Durée totale
    # 👽 title_do_agentsdo -> Agents Sessions and Pauses
    title_do_agentsdo=?Sessions et pauses des agents
    # 👽 title_do_areaando -> Area codes
    title_do_areaando=?Indicatifs régionaux
    # 👽 title_do_calldistrdo -> Call Distribution
    title_do_calldistrdo=?Répartition des appels
    # 👽 title_do_calltagdo -> Call Tags
    title_do_calltagdo=?Balises d'appel
    # 👽 title_do_casedo -> Cases
    title_do_casedo=?Cas
    # 👽 title_do_customblkdo -> Custom DataBlock
    title_do_customblkdo=?Bloc de données personnalisé
    # 👽 title_do_detailsdo -> Details
    title_do_detailsdo=?Détails
    # 👽 title_do_distrdo -> Attempts
    title_do_distrdo=?Tentatives
    # 👽 title_do_extsourcedo -> External Source
    title_do_extsourcedo=?Source externe
    # 👽 title_do_fcrdo -> First Call Resolution
    title_do_fcrdo=?Résolution au premier appel
    # 👽 title_do_groupingsdo -> Calls Grouping
    title_do_groupingsdo=?Regroupement d'appels
    # 👽 title_do_ivrdo -> Ivr Calls
    title_do_ivrdo=?Appels IVR
    # 👽 title_do_kodo -> Unanswered Calls
    title_do_kodo=?Appels sans réponse
    # 👽 title_do_okdo -> Taken Calls
    title_do_okdo=?Appels pris
    # 👽 title_do_outcomesdo -> Call with Outcomes
    title_do_outcomesdo=?Appel avec résultats
