...
## ART?
    ...
    art_deprecated_page=?Aviso: Uma nova página de agente está disponível agora. A que você está usando será removida na próxima versão. Peça ao seu administrador para migrar você para a página de agente atualizada.
...
## Headers
    ...
    hdr_dbtest_ephemeral_attribute_inspector=?Ver Atr. efémeros
    hdr_distrib=Distrib.
    hdr_distrib_day=Dia
...
### Cases
    ...
    edit_record_transcription_channel_remote_media_file=?Ficheiro de média remoto
    ...
    edit_record_transcription_channel_last_seen=?Visto pela última vez
    ...
    edit_record_transcription_server_status=?Estado
    ...
    edit_transcription_go_live_status=?Ir para a vista em direto
    ...
    edit_record_qa_aiqueues_hint=?Lista de filas separadas por barra vertical às quais isto se aplica.
...
## Quality Assessments
    ...
    qa_grading_no_valid_targets=?Por favor introduza pelo menos um alvo de classificação
...
### Automated AI grading
    ...
    qa_ai_testdialog_hint=?Este é o pedido enviado ao motor LLM, sobre uma transcrição de chamada de exemplo. Edite-o e tente novamente para ver como o modelo reage.
    ...
    qa_ai_testdialog_prompt_hint=?Prompt: esta parte depende apenas do Formulário QA, pelo que é igual em todas as chamadas e o motor LLM pode colocá-la em cache.
    # 👽 qa_ai_testdialog_request -> Request
    qa_ai_testdialog_request=?Pedido
    ...
    qa_ai_testdialog_transcript_hint=?Transcrição: esta parte muda a cada chamada, pelo que é enviada separadamente e nunca é colocada em cache.
    # 👽 qa_ai_testdialog_try -> Try it
    qa_ai_testdialog_try=?Experimentar
    ...
    qa_grader_col_transcription_duration=?Tempo de classificação
    # 👽 qa_grader_no_more_calls -> No other calls found
    qa_grader_no_more_calls=?Não foram encontradas outras chamadas
    # 👽 qa_grader_notrack_err -> You need key QA_TRACK to access this page
    qa_grader_notrack_err=?Necessita da chave QA_TRACK para aceder a esta página
    # 👽 qa_grader_stat_attempted -> Attempted
    qa_grader_stat_attempted=?Tentadas
...
## Wallboard
    ...
    rt3_manual_insert=?Inserção Manual
...
Description of configuration properties
    ...
    propedit_key_agent_leaveAllQueuesOnLogoff_desc=?Agentes serão automaticamente desconectados de todas as filas ao sair
    ...
    propedit_key_maxGatheringTime=?Define o tempo total máximo permitido para o processo de recolha ICE. Se definido como 0, a otimização é desativada.
    ...
    propedit_key_maxPostGatheringTime=?A duração do tempo limite suave acionado após a deteção de um candidato STUN/TURN encaminhável.
    ...
    propedit_key_llm_captureUniqueId_desc=?Esta expressão regular é utilizada para corresponder ao UniqueId da chamada a partir do nome do ficheiro. Ex.: audio-(.+).wav irá capturar o uniqueid 1234.1235 a partir do nome de ficheiro audio-1234.1235.wav. Se deixado em branco, nenhum UniqueId é capturado
    # 👽 propedit_key_llm_verbose_desc -> Print input and output from/to the LLM model to the system log
    propedit_key_llm_verbose_desc=?Regista no log do sistema a entrada e saída de/para o modelo LLM
    # 👽 propedit_key_qa_minPrcAllCalls_desc -> Default value of mininum percentage of calls scored for All calls on Grader's page
    propedit_key_qa_minPrcAllCalls_desc=?Valor por omissão da percentagem mínima de chamadas classificadas para "Todas as chamadas" na página do Grader
...
Description of security keys:
    ...
    keydesc_hid=?Ativar controles de fone de ouvido Softphone
    ...
    keydesc_nodialout=?O usuário não tem permissão para fazer chamadas externas
    ...
    keydesc_qa_chat_read=?Permite aos utilizadores ler o chat e a transcrição da interação
    # 👽 keydesc_qa_chat_edit -> Allows users to edit the interaction's chat and transcription
    keydesc_qa_chat_edit=?Permite aos utilizadores editar o chat e a transcrição da interação
    # 👽 keydesc_qa_chat_delete -> Allows users to delete the interaction's chat and transcription
    keydesc_qa_chat_delete=?Permite aos utilizadores eliminar o chat e a transcrição da interação
    # 👽 keydesc_qa_chat_write -> Allows users to load the interaction's chat or transcription
    keydesc_qa_chat_write=?Permite aos utilizadores carregar o chat ou a transcrição da interação
    ...
    keydesc_queue_login_assigned_only=?Os utilizadores só podem iniciar sessão nas filas que lhes estão atribuídas
    ...
    keydesc_transcription_server=?Utilizador pode ver e atualizar servidores de transcrição
    ...
    keydesc_ephemeral_read=?Os utilizadores podem ler da tabela de dados efémeros
    # 👽 keydesc_ephemeral_write -> Users can write to the ephemeral data table
    keydesc_ephemeral_write=?Os utilizadores podem escrever na tabela de dados efémeros
...
### Softphone
    ...
    phone_choose_device=?Escolher dispositivo..
    ...
    phone_hid_device_already_open=?Dispositivo HID já aberto
    ...
    phone_hid_open_device=?Selecionar um dispositivo HID
    ...
    phone_set_audio_err=?Ocorreu um erro ao configurar o dispositivo de áudio
...
## Homepage
    ...
    home_dbtest_ephemeral_attribute_inspector=?Inspetor de Atr. efémeros
    ...
    home_master_key_error=?Masterkey habilitado — uso recomendado apenas para testes
    ...
    home_transcription_title=?Transcrições em direto
    # 👽 home_transcription_subtitle -> View of Live Transcriptions
    home_transcription_subtitle=?Vista das transcrições em direto
...
These are the titles of data blocks:
    ...
    teams_blk_external_callers=?Interlocutores Externos
    # 👽 teams_blk_external_callees -> External Callees
    teams_blk_external_callees=?Interlocutores Externos Chamados
    # 👽 teams_blk_external_callers_on_all -> External Callers (on all Calls)
    teams_blk_external_callers_on_all=?Interlocutores Externos (em todas as Chamadas)
...
These are localization labels for data blocks:
    ...
    teams_avg_wait=?Esp. Méd.
    # 👽 teams_avg_talk -> Avg Talk
    teams_avg_talk=?Méd. Conversa
    # 👽 teams_tot_talk -> Tot Talk
    teams_tot_talk=?Tot. Conversa
    ...
    teams_guest_avg_wait=?Esp. Méd. Convidado
    # 👽 teams_guest_avg_talk -> Guest Avg Talk
    teams_guest_avg_talk=?Méd. Conversa Convidado
    # 👽 teams_guest_tot_talk -> Guest Tot Talk
    teams_guest_tot_talk=?Tot. Conversa Convidado
    # 👽 teams_ext_n_calls -> Ext N Calls
    teams_ext_n_calls=?N. Chamadas Ext.
    # 👽 teams_ext_avg_wait -> Ext Avg Wait
    teams_ext_avg_wait=?Esp. Méd. Ext.
    # 👽 teams_ext_avg_talk -> Ext Avg Talk
    teams_ext_avg_talk=?Méd. Conversa Ext.
    # 👽 teams_ext_tot_talk -> Ext Tot Talk
    teams_ext_tot_talk=?Tot. Conversa Ext.
...
### Viewer of ephemeral attributes
    ...
    ephatt_attribute_confirm_delete=?Isto irá eliminar este atributo
...
Other items
    ...
    err_not_blank=?Campo não pode ficar em branco.
    ...
    licence_licence_expiring=?A sua licença está prestes a expirar
    ...
    qap_agent_license_error=?Excede o limite de agentes licenciados. Por favor contacte o seu supervisor.
    # 👽 qap_amo -> AMO
    qap_amo=?AMO
    ...
    qap_logout_message=?Desconectando você de todas as filas
    # 👽 qap_memos -> Memos
    qap_memos=?Memos
    ...
    qap_extended_status=?Estado alargado
    # 👽 qap_number_dialed -> Number dialed
    qap_number_dialed=?Número marcado
    ...
    qap_wombat_dial=?Marcar
    # 👽 qap_wombat_ext_status_success -> Extended status added successful
    qap_wombat_ext_status_success=?Estado alargado adicionado com sucesso
    ...
    qap_wombat_update_disable_msg=?Deve ser selecionado um estado alargado
    ...
    qap_id_found_wrong_state=?O ID existe, mas a chamada está num estado incorreto
    ...
    rt3_att_text=?Efémero - Texto
    # 👽 rt3_nofstate_unix_wbt -> Ephemeral - Time
    rt3_nofstate_unix_wbt=?Efémero - Hora
    ...
    rt3_nofstate_wbt=?Efémero - Valor
    ...
    rt3_attributes=?Atributo efémero
    ...
    reports_filter_empty_hour_toggle=?Ocultar períodos horários vazios
    ...
    reports_datablock_info_error=?Não foi possível carregar a informação do DataBlock
    ...
    reports_email_not_valid=?Introduza endereços de e-mail válidos
    ...
    reports_exportjob_create=?Criar Novo Job de Exportação
    # 👽 reports_exportjob_create_successful -> Export job created successfully
    reports_exportjob_create_successful=?Job de exportação criado com sucesso
    # 👽 reports_exportjob_error_create -> Error Creating Export Job:
    reports_exportjob_error_create=?Erro ao Criar Job de Exportação:
    # 👽 reports_exportjob_title -> Existing jobs for this report
    reports_exportjob_title=?Jobs existentes para este relatório
    ...
    reports_qa_grader_type=?Tipo de classificador
    ...
    reports_qa_tracked_by=?Classificado por
    ...
    reports_save_report_title_exception=?Um relatório com o mesmo nome já existe. Selecione um nome diferente.
    ...
    reports_qa_noteform_save=?Guardar nota
    ...
    reports_delete_filter_text=?Eliminar filtro
    ...
    sync2_delete_cluster=?Eliminar cluster
    ...
    timeline_chat_delete_confirm=?A conversa selecionada será eliminada. Continuar?
    # 👽 timeline_chat_edit_speakers -> Edit Speakers
    timeline_chat_edit_speakers=?Editar oradores
    ...
    timeline_chat_segments_delete=?Eliminar conversa
    # 👽 timeline_chat_speaker -> Speaker
    timeline_chat_speaker=?Orador
    ...
    timeline_download_chat_file=?Transferir ficheiro
    ...
    timeline_no_chats=?Nenhuma conversa disponível
    ...
    timeline_tab_chats=?Conversas
    # 👽 timeline_tab_chats_speaker_error -> Speaker names must be unique
    timeline_tab_chats_speaker_error=?Os nomes dos oradores devem ser únicos
    ...
    title_do_calltagdo=?Etiquetas de Chamada
    # 👽 title_do_casedo -> Cases
    title_do_casedo=?Casos
    # 👽 title_do_customblkdo -> Custom DataBlock
    title_do_customblkdo=?DataBlock Personalizado
    ...
    title_do_outcomesdo=?Chamada com Resultados
